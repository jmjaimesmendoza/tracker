<script lang="ts">
  ...
  let km: number = 0;
  let selectedJob: { index: string; value: string; label: string };
  let jobItems = [
    "Preventivo",
    "Reparacion leve",
    "Reparacion mayor",
    ...
    <div class="flex flex-col mt-8">
      {#if selectedOption === "D"}
        ...
        <input
          required
          id="target"
          class="border border-gray-300 rounded-md px-3 py-2 mt-1 focus:outline-none focus:border-green-500 h-12"
          type="date"
          name="target"
          bind:value={target}
        />
