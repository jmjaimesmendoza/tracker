<script lang="ts">
  ...
  import { parsedLogStore } from "../../stores/logStore";
  ...
  const showPopupWithProps = (logId: number) => {
    modal.set(
      bind(EditLogForm, {
        id: logId,
      })
  ...
  const table = createTable(parsedLogStore, {
          ...
          >
            ...
            >
              <button
                ...
                on:click={() => showPopupWithProps(row.original.id)}
              >
